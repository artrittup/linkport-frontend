...
export default function CompanyTalentCard({ member }) {
  ...
  return (
    <article className="flex h-full min-w-0 flex-col rounded-2xl border border-[#233554] bg-[#112240]/65 p-5">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          {(member.university || member.location) && <p className="mt-1 break-words text-xs text-[#64748b]">{[member.university, member.location].filter(Boolean).join(' · ')}</p>}
        </div>
      </div>

      {member.collaborationStatus && (
        <span className="mt-4 w-fit max-w-full break-words rounded-full border border-[#64ffda]/25 bg-[#64ffda]/5 px-2.5 py-1 text-xs text-[#64ffda]">
      ...
      {visibleSkills.length > 0 && <div className="mt-4 flex flex-wrap gap-2">
        {visibleSkills.map((skill) => <span key={skill} className="max-w-full break-words rounded-md border border-[#233554] px-2.5 py-1 text-xs text-[#a8b2d1]">{skill}</span>)}
      ...
      {member.interests.length > 0 && <div className="mt-4 flex flex-wrap gap-2">
        {member.interests.slice(0, 2).map((interest) => <span key={interest} className="max-w-full break-words text-xs text-[#8892b0]">{getInterestLabel(interest)}</span>)}
      </div>}

      <div className="mt-auto pt-5">
        <Link to={`/company/talent/${member.id}`} className="inline-flex w-full items-center justify-center rounded-lg bg-[#64ffda] px-4 py-2.5 text-sm font-semibold text-[#071426] hover:bg-[#7dffe1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#64ffda]">
          View profile
        </Link>
      </div>
    </article>
