    ...
    <CandidateLayout title={member.name}>
      <div className="min-w-0 max-w-full">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {member.isCurrentUser && <Link to="/member/profile" className="text-sm font-medium text-primary hover:underline">View my editable profile</Link>}
        </div>
        ...
        <div className="mt-6 grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(16rem,0.65fr)]">
          <ProfileSection title="About">
            ...
            {member.interests.length > 0 && <div className="mt-5"><h4 className="text-xs uppercase tracking-wide text-text-subtle">Current interests</h4><div className="mt-2 flex min-w-0 flex-wrap gap-2">{member.interests.map((interest) => <span key={interest} className="max-w-full break-words rounded-full border border-border px-3 py-1 text-xs text-text-secondary">{getInterestLabel(interest)}</span>)}</div></div>}
          </ProfileSection>

          <ProfileSection title="Collaboration">
            {member.collaborationStatus ? <p className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-sm text-primary">{getCollaborationStatusLabel(member.collaborationStatus)}</p> : <p className="text-sm text-text-muted">No collaboration status has been shared.</p>}
            ...
            {member.interests.length > 0 && <div className="mt-5"><h4 className="text-xs uppercase tracking-wide text-text-subtle">Interest areas</h4><p className="mt-2 break-words text-sm leading-6 text-text-secondary">{member.interests.map(getInterestLabel).join(', ')}</p></div>}
            {!hasExternalLinks && <p className="mt-5 text-sm leading-6 text-text-muted">No external professional links have been shared.</p>}
          </ProfileSection>
        </div>

        <div className="mt-6">
          <ProfileSection title="Skills">
            {member.skills.length > 0 ? <div className="flex min-w-0 flex-wrap gap-2">{member.skills.map((skill) => <span key={skill} className="max-w-full break-words rounded-full border border-primary/25 bg-primary/5 px-3 py-1.5 text-sm text-primary">{skill}</span>)}</div> : <p className="text-sm text-text-muted">No skills have been shared.</p>}
          </ProfileSection>
        </div>

        <section className="mt-6 min-w-0">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div><h3 className="text-2xl font-semibold text-text-primary">Projects</h3><p className="mt-2 text-sm text-text-muted">{member.projectCount} community {member.projectCount === 1 ? 'project' : 'projects'} shared.</p></div>
            <Link to="/member/projects" className="text-sm font-medium text-primary hover:underline">Explore projects</Link>
          </div>
          {member.projects.length > 0 ? (
            ...
            <div className="mt-5 rounded-2xl border border-border bg-surface/55 p-6"><p className="text-sm text-text-muted">This member has not shared a Community Project yet.</p></div>
          )}
        </section>
      </div>
    </CandidateLayout>
