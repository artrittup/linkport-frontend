...
export default function CompanyApplications() {
  ...
  const updateStatus = (id, nextStatus) => setApplications((current) => current.map((item) => item.id === id ? { ...item, status: nextStatus } : item))
    ...
    <DashboardLayout title="Applications" navItems={navItems} userType="Company">
      <div className="space-y-8">
        ...
        <section><p className="mb-4 text-sm text-[#8892b0]">Showing <span className="text-[#e6f1ff]">{filtered.length}</span> applications</p>
          ...
          {filtered.length === 0 && <Card padding="lg" className="text-center"><p>No applications match your filters.</p></Card>}
        </section>
      </div>
