    ...
    <DashboardLayout title="Manage Jobs" userType="Company">
      <div className="space-y-8">
        <section className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><p className="font-mono text-sm text-primary">Hiring workspace</p><h2 className="mt-2 text-2xl font-bold tracking-tight sm:text-3xl">Manage Jobs</h2><p className="mt-3 text-text-muted">Create, edit, and manage your company job opportunities.</p></div>
          <Button size="lg" onClick={openCreateForm}>Post New Job</Button>
        </section>

        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((item, index) => <Card key={item.label} hover><div className="flex justify-between"><div><p className="text-sm text-text-muted">{item.label}</p><p className="mt-3 text-3xl font-bold">{item.value}</p></div><span className="font-mono text-xs text-primary">0{index + 1}</span></div></Card>)}
        </section>
        ...
        <section>
          <div className="mb-5"><h2 className="text-xl font-semibold sm:text-2xl">Company Jobs</h2><p className="mt-1 text-sm text-text-muted">{pagination.total ?? 0} jobs in your company workspace.</p></div>
          {isLoading ? <LoadingSpinner label="Loading company jobs..." size="lg" /> : error ? <EmptyState title="Unable to load jobs" description={error} /> : filteredJobs.length === 0 ? <EmptyState title="No jobs found" description={jobs.length ? 'Try changing your search.' : 'Create your first company job.'} /> : (
            <>
              <Card padding="sm" className="hidden overflow-hidden md:block"><div className="overflow-x-auto"><table className="w-full min-w-[950px] text-left"><thead><tr className="border-b border-border text-[11px] uppercase text-text-subtle"><th className="p-4">Job title</th><th className="p-4">Location</th><th className="p-4">Type</th><th className="p-4">Deadline</th><th className="p-4">Applications</th><th className="p-4">Status</th><th className="p-4 text-right">Actions</th></tr></thead><tbody>{filteredJobs.map((job) => <tr key={job.id} className="border-b border-border/70 last:border-0"><td className="p-4 text-sm font-medium">{job.title}</td><td className="p-4 text-xs text-text-muted">{job.location}</td><td className="p-4 text-xs text-text-muted">{job.type || 'Not specified'}</td><td className="p-4 text-xs text-warning">{job.deadline || 'No deadline'}</td><td className="p-4">{job.applications}</td><td className="p-4"><StatusBadge status={job.status} /></td><td className="p-4"><div className="flex justify-end gap-2"><Button variant="ghost" size="sm" onClick={() => setSelectedJob(job)}>View</Button><Button variant="outline" size="sm" onClick={() => openEditForm(job)}>Edit</Button><Button variant="danger" size="sm" disabled={deletingId === job.id} onClick={() => deleteSelectedJob(job)}>{deletingId === job.id ? 'Deleting...' : 'Delete'}</Button></div></td></tr>)}</tbody></table></div></Card>
              <div className="space-y-4 md:hidden">{filteredJobs.map((job) => <Card key={job.id} hover><div className="flex justify-between"><div><h3 className="font-semibold">{job.title}</h3><p className="mt-1 text-sm text-primary">{job.type || 'Type not specified'}</p></div><StatusBadge status={job.status} /></div><div className="my-4 grid grid-cols-2 gap-3 border-y border-border py-4 text-xs text-text-muted"><p>{job.location}</p><p>{job.deadline || 'No deadline'}</p><p>{job.applications} applications</p></div><div className="grid grid-cols-3 gap-2"><Button variant="ghost" size="sm" onClick={() => setSelectedJob(job)}>View</Button><Button variant="outline" size="sm" onClick={() => openEditForm(job)}>Edit</Button><Button variant="danger" size="sm" disabled={deletingId === job.id} onClick={() => deleteSelectedJob(job)}>Delete</Button></div></Card>)}</div>
            </>
          )}
          {!isLoading && !error && lastPage > 1 && <div className="mt-6 flex items-center justify-center gap-4"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((current) => current - 1)}>Previous</Button><span className="text-sm text-text-muted">Page {pagination.current_page} of {lastPage}</span><Button variant="outline" size="sm" disabled={page >= lastPage} onClick={() => setPage((current) => current + 1)}>Next</Button></div>}
        </section>
      </div>

      <Modal isOpen={Boolean(selectedJob)} onClose={() => setSelectedJob(null)} eyebrow="Job details" title={selectedJob?.title ?? 'Job details'}>
        {selectedJob && <><DetailGrid items={[
          { label: 'Status', value: selectedJob.status }, { label: 'Type', value: selectedJob.type ?? 'Not specified' },
          { label: 'Location', value: selectedJob.location }, { label: 'Deadline', value: selectedJob.deadline || 'No deadline' },
          { label: 'Applications', value: selectedJob.applications },
